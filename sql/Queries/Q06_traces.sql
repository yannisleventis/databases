...
SELECT 
    ...
    COUNT(r.Rating_ID) AS Rating_Count
FROM 
    Visitor v USE INDEX (PRIMARY)
    JOIN Ticket t USE INDEX (idx_ticket_visitor) ON v.Visitor_ID = t.Visitor_ID
    JOIN Event e USE INDEX (PRIMARY) ON t.Event_ID = e.Event_ID
    JOIN Performance p USE INDEX (idx_performance_event) ON e.Event_ID = p.Event_ID
    JOIN Stage s USE INDEX (PRIMARY) ON p.Stage_ID = s.Stage_ID
    JOIN Performance_Type pt USE INDEX (PRIMARY) ON p.Performance_Type_ID = pt.Performance_Type_ID
    LEFT JOIN Artist a USE INDEX (PRIMARY) ON p.Artist_ID = a.Artist_ID
    LEFT JOIN Band b USE INDEX (PRIMARY) ON p.Band_ID = b.Band_ID
    LEFT JOIN Rating r USE INDEX (idx_rating_performance) ON p.Performance_ID = r.Performance_ID AND v.Visitor_ID = r.Visitor_ID
WHERE 
    v.Visitor_ID = '69'  
    AND t.Ticket_Status_ID = (SELECT Status_ID FROM Ticket_Status WHERE Status_Name = 'activated')
GROUP BY 
...
SELECT /*+ MERGE_JOIN(v, t, e, p, s, pt, a, b, r) */
    v.Visitor_ID,
    CONCAT(v.First_Name, ' ', v.Last_Name) AS Visitor_Name,
    p.Performance_ID,
    CASE 
...
SELECT /*+ HASH_JOIN(v, t, e, p, s, pt, a, b, r) */
    v.Visitor_ID,
    CONCAT(v.First_Name, ' ', v.Last_Name) AS Visitor_Name,
    p.Performance_ID,
    CASE 
